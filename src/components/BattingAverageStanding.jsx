...
import { getBattingAverageStanding, getPlayerById } from '../services/mlbApi';
    ...
    <div className="batting-average-standing">
      ...
      {battingAverageStanding.slice(0, 9).map((p) => (
        <div className="player-row" key={p.person.id}>
          ...
          <span>
            <div
              className="player-name_bas"
              onClick={() => handlePlayerClick(p.person.id)}
            >
              {p.person.fullName}
            </div>
            <div className="team-abbr_bas">
              {TEAM_NAME_ABBREVIATION[p.team.id]}
            </div>
          </span>
          <span>{p.value}</span>
        </div>
